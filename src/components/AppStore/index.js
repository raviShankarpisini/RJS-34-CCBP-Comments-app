...
const sampleList = appsList.filter(each => each.category === 'SOCIAL')
...
class AppStore extends Component {
  ...
  onClickTab = tabId => {
    const tabClickResultList = appsList.filter(
      eachCategory => eachCategory.category === tabId,
    )
    this.setState({resultList: tabClickResultList, activeTab: tabId})
  }

  render() {
    const {searchInput, resultList, activeTab} = this.state

    const filteredSearchList = resultList.filter(eachApp =>
